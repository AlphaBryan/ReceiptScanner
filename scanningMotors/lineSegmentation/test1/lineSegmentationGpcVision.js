...
function init(data, wordsToRemove = null) {
  ...
  if (wordsToRemove) {
    mergedArray = coordinatesHelper.removeStringsFromData(
      mergedArray,
      wordsToRemove
    );
  }
  ...
  while (lines.length !== 1) {
    ...
    while (true) {
      ...
      if (l === "") {
        ...
        mergedElement.boundingPoly.vertices[1] =
          wElement.boundingPoly.vertices[1];
        mergedElement.boundingPoly.vertices[2] =
          wElement.boundingPoly.vertices[2];
...
function arrangeWordsInOrder(mergedArray, k) {
  ...
  lines.sort((a, b) => {
    ...
    const isFirstGreater =
      _1BoudingPoly.vertices[0].x > _2BoudingPoly.vertices[0].x;
